...
def sample_dataset_from_model(
    ...
    # Create dataset name.
    dataset_name = f"{model_name_or_path.split('/')[1]}_temp={temperature}_max_seq_length={max_seq_length}"
    ...
    while len(dataset_prompts) < total_num_samples:
        ...
        for sample in batch_samples:
            # Only keep outputs that have both a user prompt and assistant response.
            if not sample.startswith("user: "):
                continue
            if "assistant:" not in sample:
                continue

            raw_prompt, raw_response = sample.split("assistant:")
    ...
    dataset = dataset.map(compute_gold_reward_model_preferences, batched=False)

    # Push the dataset to HuggingFace.
    commit_info = dataset.push_to_hub(repo_id=dataset_name)
